define(function (require, exports, module) {
    module.exports = function (skin, options, solo) {
        ...
        var keymap = skin.create(require('components/keymap'), solo.expand({
            ...
            '0000:13': function (context) { // enter
                if (context.isCanvasVisible) {
                    if (ixml) {
                        var stepInfo = ixml.getProcedureItemInfo(procedure.getContextItemId()),
                            children = stepInfo.raw['@children']
                                .map(function (node) {
                                    return node.$attr('id');
                                })
                                .filter(solo.app.procedure.isValidContextItem);
                        if (children.length) {
                            procedure.seekToSubstep(children[0]);
                        }
                    }
                }
            },
            '0000:8': function (context) { // backspace
                if (context.isCanvasVisible) {
                    if (ixml) {
                        var stepInfo = ixml.getProcedureItemInfo(procedure.getContextItemId());
                        if (stepInfo.parent) {
                            procedure.seekToSubstep(stepInfo.parent);
                        }
                    }
                }
            },
            ...
            '0010:76': function (context) { // L + shift
                if (context.isCanvasVisible) {
